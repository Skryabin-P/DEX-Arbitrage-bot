...
uniswap_v3 = UniswapV3('Ethereum', 'MAINNET', fee=3000, web3_provider='http://127.0.0.1:7545', slippage=0.5)
...
routers = [uniswap_v3.router.address, uniswap_v3.router.address, uniswap_v3.router.address]
trades = [str.encode(buy_weth_encoded), str.encode(buy_link_encoded), str.encode(sell_link_encoded)]
tokens = [weth_usdt['quote_asset'].address, link_weth['quote_asset'].address, link_weth['base_asset'].address]
approves = [str.encode(usdt_approve), str.encode(weth_approve), str.encode(link_approve)]
...
call_function = uniswap_v3.arbitrage_contract.functions.requestFlashLoan(
    ...
    encoded_data).build_transaction({"chainId": chain_id,
                                    "from": my_address, "nonce": nonce, "gas": 6721975, "gasPrice":gas_price})
